...
```
   - `schema_version`: always "0.2" for new exports
   ...
   - `subagents` array: include only if subagent files were found
     ...
     - `session_id`: extracted from first line of subagent JSONL (`"sessionId":"..."`)
     - `session_num`: map session_id to phase order (1, 2, 3...) for easy correlation with session-N.jsonl
